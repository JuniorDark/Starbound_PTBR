...
function updateToolTip(position)
  ...
  if button ~= nil then
    ...
    if string.len(button) > 11 and string.sub(button, 1, 12)  == "templateSlot" then
      self.toolTip = "Escaneia a placa no espaço de entrada do dispensador de placas"
      blankTip = false
    ...
    elseif button == "pixelPress" then
      if self.currentCursorModifier == "pickPixel" then
        self.toolTip = "[Clique Esquerdo] para escolher a cor"
      elseif self.currentCursorModifier == "fillPixel" then
        self.toolTip = "[Clique Esquerdo] para preencher"
      else
        self.toolTip = "[Clique Esquerdo] para desenhar, [Clique Direito] para apagar"
      end
      blankTip = false
    elseif button == "undoPress" then
      if #self.undoStack > 0 then
        self.toolTip = "Desfazer ações anteriores ("..(undoTips[self.undoStack[#self.undoStack][1]] or "FALTANDO")..")"
        blankTip = false
      end
    else
      --most come from the array in customeasel-data.lua
      self.toolTip = toolTips[button] or "*TOOLTIP FALTANDO*"
      blankTip = false
    end
  end
...
function assignInputString(stringType, input)
  ...
  if isInvalid then
    self.thrownError = {300, "ERRO! ARGUMENTO INVÁLIDO", "OBEDEÇA CRITÉRIOS DADOS"}
  end
end
...
function outputSign()
  ...
  if not outputSuccess then
    self.thrownError = {500, "   ERRO! NANO-PAPEL ATOLADO", "     POR FAVOR LIMPAR SAÍDA"}
  end
  --sb.logInfo("---------------------------------")
end
